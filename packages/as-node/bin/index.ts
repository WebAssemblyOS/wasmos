...
import { Host, ASImport, Env } from "assemblyscript/lib/host";
...
async function loadBinary<T extends Binary>(binPath: string, imports: any): Promise<T & loader.ASInstance> {
  let bin = await promisfy<Buffer>(fs.readFile)(binPath);
  let instance = loader.instantiateBuffer<T>(bin, imports)
  return instance;
}

async function exec(filename: string, cwd: string = process.cwd()) {
