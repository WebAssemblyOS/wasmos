...
interface CompilerOptions {
  ...
  writeFile?: (filename: string, contents: Uint8Array, baseDir: string) => Promise<void>;
...
export class Compiler {
  static opts(): CompilerOptions {
    return {
      ...
      writeFile: async (basename: string, content: Uint8Array, baseDir: string) => {
        let base = baseDir ? baseDir : "";
        let file = path.join(base, basename);
  ...
  static async compileOne(bin: string): Promise<void> {
    let opts = this.opts();
    ...
    (<any>asc).main(
      ["preamble.ts",
        ...
        "--binaryFile", `${opts.outDir}/${folder}/index.wasm`,
        "--textFile", `${opts.outDir}/${folder}/index.wat`,
        "--tsdFile", `${opts.outDir}/${folder}/index.d.ts`,
        "--importMemory",
        "--measure",
        "--validate",
        "--debug",
